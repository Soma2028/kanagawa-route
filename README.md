...
上記のモデルは、全9駅を「鎌倉駅からの一直線」として距離の絶対差で乗車時間を
計算していた。しかし実際は北鎌倉（横須賀線）と、それ以外7駅（江ノ電）は
別々の路線で、鎌倉駅で接続する構造になっている。この区別がなかったため、
**北鎌倉→長谷が乗り換えなしの直接区間として1分**と算出されるなど、
路線をまたぐ区間が大幅に過小評価されていた。

- 各駅に路線（江ノ電/横須賀線/鎌倉＝共通）を持たせた
- 路線が異なる区間は、鎌倉駅での乗り換え時間（**5分、ホーム移動＋2本目の
  待ち時間を含めた想定値**）を挟んだ合算に変更した
- 待ち時間も路線別にした。江ノ電は運行間隔12分（待ち時間6分）、横須賀線は
  15分（待ち時間7.5分）とした。ただし**横須賀線の間隔は正確なデータがなく、
  「江ノ電よりやや疎」という前提の推定値**であり、確認が取れ次第見直す

是正後、**北鎌倉→長谷は 4分(北鎌倉→鎌倉) + 5分(乗り換え) + 5分(鎌倉→長谷) = 14分**
になった。

また、駅間の移動時間を「徒歩と鉄道を比較して短い方を採用する」ロジックが
`travel_time.py` の行列構築・`optimize.py` の起点用・昼食用の計算の3箇所に
重複していたため、`travel_time.best_travel()` に一本化した（乗車駅によって
待ち時間が変わり得るため、往復で対称になるとは限らない点にも対応）。
...
行程カードのエリア色帯を `border-top` で描いていたため、角丸を 10px → 20px に
広げた際に、角のカーブ部分で帯がテーパーして両端が三角に尖った。丸みを足した
つもりが鋭角を生んでいたことになる。`border-radius` に完全追従する
`box-shadow: inset 0 4px 0` に置き換えて解決。
...
- 横須賀線の運行間隔（現在15分の推定値）を実際の時刻表で確認する
